...
| 音声 | 札で示し、既定のアプリへ渡す |
| CSV・TSV | 簡単な表計算のように開く。並べ替え・範囲の選択・写しができる |
...
CSV と TSV は、文章の一部としてではなく「使うもの」として開きます。

- 1行目を見出しにするかを、その場で切り替えられます
- 見出しを押すと、その列で並べ替わります（昇順 → 降順 → 元の順）
- ます目を引きずると範囲で選べ、`Ctrl`/`⌘` + `C` でタブ区切りとして写せます
  （そのまま Excel や Google スプレッドシートへ貼れます）
- 数値は右へ寄り、行の番号と縦線が常に出ます
